...
async def backup_database():
    ...
    db_name = os.getenv("DB_NAME")  # Замените на имя вашей базы данных
    db_user = os.getenv("DB_USER")        # Замените на имя пользователя базы данных
    db_password = os.getenv("DB_PASSWORD")  # Замените на пароль пользователя базы данных
...
async def analytics_dashboard():
    """
    Общая страница аналитики.
    """
    start_date = request.args.get('start_date', None)
    end_date = request.args.get('end_date', None)
    group_by = request.args.get('group_by', 'day')  # По умолчанию группировка по дням

    top_sales = revenue = reviews_by_category = []
    ...
    try:
        ...
        if start_date and end_date:
            top_sales = await get_top_sales(current_app.db_pool, start_date, end_date, limit=10)
            revenue = await get_analytics(
        ...
        reviews_by_category = await get_reviews_by_category(current_app.db_pool)
        categories = await get_all_categories(current_app.db_pool)

    except Exception as e:
        current_app.logger.error(f"Ошибка при выполнении аналитики: {e}")
        await flash('Произошла ошибка при загрузке данных.', 'danger')

    # Отладочные сообщения
    current_app.logger.info(f"Top Sales: {top_sales}")
    current_app.logger.info(f"Revenue: {revenue}")
    current_app.logger.info(f"Reviews by Category: {reviews_by_category}")
